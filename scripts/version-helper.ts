...
function executeGitOperations(version: string, _updatedFiles: number): void {
...
function showVersions(): void {
  ...
  try {
    ...
    console.log(
      `❌ Package.json: Error reading version (${(error as Error).message})`
    );
  }
}
...
if (command === 'update') {
  if (!newVersion) {
    ...
    console.log(
      `🔄 Auto-incrementing version: ${currentVersion} → ${nextVersion}`
    );
  ...
  if (versionPattern.test(command)) {
    ...
    console.log('Usage:');
    console.log(
      '  npx tsx scripts/version-helper.ts show              # Show current version'
    );
    console.log(
      '  npx tsx scripts/version-helper.ts version           # Show current version'
    );
    console.log(
